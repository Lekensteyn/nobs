...
var (
	...
	bench_x              = FpElement{17026702066521327207, 5108203422050077993, 10225396685796065916, 11153620995215874678, 6531160855165088358, 15302925148404145445, 1248821577836769963, 9789766903037985294}
	bench_y              = FpElement{4227467157325093378, 10699492810770426363, 13500940151395637365, 12966403950118934952, 16517692605450415877, 13647111148905630666, 14223628886152717087, 7167843152346903316}
	bench_z              = FpElementX2{1595347748594595712, 10854920567160033970, 16877102267020034574, 12435724995376660096, 3757940912203224231, 8251999420280413600, 3648859773438820227, 17622716832674727914, 11029567000887241528, 11216190007549447055, 17606662790980286987, 4720707159513626555, 12887743598335030915, 14954645239176589309, 14178817688915225254, 1191346797768989683}
)
...
func AddProjFp2(xP, xQ, xPmQ *ProjectivePoint) ProjectivePoint {
	...
	Add(&v0, &xP.X, &xP.Z) // X_P + Z_P
	Sub(&v1, &xQ.X, &xQ.Z)
	Mul(&v1, &v1, &v0)     // (X_Q - Z_Q)(X_P + Z_P)
	Sub(&v0, &xP.X, &xP.Z) // X_P - Z_P
	Add(&v2, &xQ.X, &xQ.Z)
	Mul(&v2, &v2, &v0) // (X_Q + Z_Q)(X_P - Z_P)
	Add(&v3, &v1, &v2)
	Square(&v3, &v3) // 4(X_Q X_P - Z_Q Z_P)^2
	Sub(&v4, &v1, &v2)
	Square(&v4, &v4)         // 4(X_Q Z_P - Z_Q X_P)^2
	Mul(&v0, &xPmQ.Z, &v3)   // 4X_{P-Q}(X_Q X_P - Z_Q Z_P)^2
	Mul(&xR.Z, &xPmQ.X, &v4) // 4Z_{P-Q}(X_Q Z_P - Z_Q X_P)^2
	xR.X = v0
	return xR
}
...
func DoubleProjFp2(xP *ProjectivePoint, Aplus2C, C4 *Fp2Element) ProjectivePoint {
	...
	Add(&v1, &xP.X, &xP.Z) // (X+Z)^2
	Square(&v1, &v1)
	Sub(&v2, &xP.X, &xP.Z) // (X-Z)^2
	Square(&v2, &v2)
	Sub(&xz4, &v1, &v2)     // 4XZ = (X+Z)^2 - (X-Z)^2
	Mul(&v2, &v2, C4)       // 4C(X-Z)^2
	Mul(&xQ.X, &v1, &v2)    // 4C(X+Z)^2(X-Z)^2
	Mul(&v3, &xz4, Aplus2C) // 4XZ(A + 2C)
	Add(&v3, &v3, &v2)      // 4XZ(A + 2C) + 4C(X-Z)^2
	Mul(&xQ.Z, &v3, &xz4)   // (4XZ(A + 2C) + 4C(X-Z)^2)4XZ
	// Now (xQ.x : xQ.z)
...
func ScalarMult(curve *ProjectiveCurveParameters, xP *ProjectivePoint, scalar []uint8) ProjectivePoint {
	...
	Add(&Aplus2C, &curve.C, &curve.C) // = 2*C
	Add(&C4, &Aplus2C, &Aplus2C)      // = 4*C
	Add(&Aplus2C, &Aplus2C, &curve.A) // = 2*C + A
	...
	for i := len(scalar) - 1; i >= 0; i-- {
		...
		for j := 7; j >= 0; j-- {
			...
			CondSwap(&x0.X, &x0.Z, &x1.X, &x1.Z, (bit ^ prevBit))
	...
	CondSwap(&x0.X, &x0.Z, &x1.X, &x1.Z, prevBit)
	return x0
}
